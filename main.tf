...
resource "null_resource" "deploy_docker" {
  provisioner "remote-exec" {
    inline = [
      ...
      "echo 'docker run -e SPRING_PROFILES_ACTIVE=qa -e CRUD_DB_URL=${var.crud_db_url} -e CRUD_DB_USERNAME=${var.crud_db_username} -e CRUD_DB_PASSWORD=${var.crud_db_password} -d -p $random_port:8080 ${var.docker_image}:latest'",
